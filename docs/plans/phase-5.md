...
- **Disambiguation suggestions (Phase 3 R4).** A freshly-mentioned entity
  whose name closely matches an existing one (character-trigram Dice ≥ 0.5)
  raises a "did you mean [[X]]?" banner in the staging tray. Accepting it
  merges the staged fact onto the existing entity and re-routes the change;
  dismissing keeps them distinct. `lookup_entity` itself stays exact-match —
  the check runs only when an entity is otherwise treated as new.
  `core::similarity`, `MemoryStore::similar_entities`, `apply_disambiguation`.
